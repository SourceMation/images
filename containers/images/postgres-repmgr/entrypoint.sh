...
if [ "$REPMGR_ROLE" = 'primary' ]; then
  ...
  if [ -z "$REPMGR_UPSTREAM_NAME" ]; then
      echo "ERROR: REPMGR_UPSTREAM_NAME environment variable is not set."
      exit 1
  fi
  ...
  if [ ! -s "$PGDATA/PG_VERSION" ]; then
      ...
      echo "--- Waiting for primary node ($REPMGR_UPSTREAM_NAME) to be ready ---"
      ...
      repmgr -h "$REPMGR_UPSTREAM_NAME" -U "$REPMGR_USER" -d "$REPMGR_DB" -f "$REPMGR_CONFIG_FILE" standby clone
