...
if [ -z "${TOMCAT_VERSION:-}" ]; then
  echo "Failed to determine latest Tomcat 9 version" >&2
  exit 1
fi

echo "Latest version of $APP is $TOMCAT_VERSION"

# Get corresponding SHA512 checksum for this version
...
if [ -z "${TOMCAT_SHA512:-}" ]; then
  echo "Failed to determine SHA512 checksum for Tomcat ${TOMCAT_VERSION}" >&2
  exit 1
fi
...
sed -i "s/ENV TOMCAT_SHA512=[0-9a-fA-F]*/ENV TOMCAT_SHA512=$TOMCAT_SHA512/" Dockerfile || exit 1
