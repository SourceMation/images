#!/usr/bin/env bash
# ---------------------------------------------------
# Automated build process for the python 3.8 image
...
VERSION=$(git ls-remote --refs --tags https://github.com/python/cpython.git | grep -o 'v3.8.[0-9]*$'| tr -d 'v' | sort --version-sort --reverse | head -1)
