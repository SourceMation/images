#!/usr/bin/env bash
# ---------------------------------------------------
# Automated build process for the GCC 17 - snapshot
...
url="https://ftp.gwdg.de/pub/misc/gcc/snapshots/LATEST-17/"

# Download the HTML content
html_content=$(wget -qO- "$url")

# Extract the link using a more robust pattern than simple grep
latest_file=$(echo "$html_content" | sed -n 's/.*<a href="\([^"]*\)">gcc-17-[0-9]\{8\}\.tar\.xz<\/a>.*/\1/p')

if [ -n "$latest_file" ]; then
  latest_url="$url$latest_file"
  echo "The latest GCC 17 snapshot URL is: $latest_url"
else
  echo "Could not find the latest GCC 17 snapshot link."
fi

# Extract the version number from the filename
# The version number is in the format "gcc-17-YYYYMMDD.tar.xz"
# We want to extract "YYYYMMDD"
VERSION=$(echo "$latest_file" | sed -E 's/gcc-17-([0-9]{8})\.tar\.xz/\1/')
