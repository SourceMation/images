...
This image provides the Java Runtime Environment (JRE) 8 from the Eclipse
Temurin project, built as a compilation of the OpenJDK project.

This optimized Docker image offers a clean and lightweight JRE 8 installation.
Built on Sourcemation's streamlined Debian 13 Slim base, it ensures minimal
resource usage and improved security.

This image serves as a drop-in replacement for the Temurin JRE 8 image.
...
```bash
docker run -it --rm --name jre8 sourcemation/jre-8
```

This launches a shell within a container that has JRE 8 installed and ready
for use.
...
```bash
docker run -it --rm -v /path/to/your/application:/app sourcemation/jre-8
...
```bash
docker run -v /home/dev/mycerts:/certificates/ -e USE_SYSTEM_CA_CERTS=1 sourcemation/jre-8
```
...
the Temurin JRE 8 image. **This image runs as the root user! You should
...
are available on the [Sourcemation platform](https://www.sourcemation.com/catalog/jre-8).
