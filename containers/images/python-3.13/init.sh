#!/usr/bin/env bash
# ---------------------------------------------------
# Automated build process for the python 3.13 image
...
VERSION=$(git ls-remote --refs --tags https://github.com/python/cpython.git | grep  -o 'v3.13.[0-9]*$'| tr -d 'v' | sort --version-sort --reverse | head -1)
