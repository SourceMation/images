# Java Development Kit (JDK) 8 Container Image

This Docker image provides the Java Development Kit (JDK) 8 from the Eclipse
...
image delivers a secure and efficient JDK 8 installation with minimal
footprint and enhanced security features.

Use this container image to run and build Java applications as a reliable
drop-in replacement for the Eclipse Temurin JDK 8 image.

## Getting Started

To quickly run the JDK 8 Docker image with default settings, use the following
command:
...
Here's how to run a pre-built Java application using this JDK 8 container
image with a simple Dockerfile:

```Dockerfile
FROM sourcemation/jdk-8
...
```bash
docker run -it --rm -v /path/to/your/application:/app sourcemation/jdk-8 java -jar /app/your-application.jar
```

Lastly, to add custom CA certificates, you can mount a volume (or copy it to
the image during the Containerfile/Dockerfile build) and set the
USE\_SYSTEM\_CA\_CERTS environment variable to 1:

```bash
docker run -v /home/dev/mycerts:/certificates/ -e USE_SYSTEM_CA_CERTS=1 sourcemation/jdk-8
```
...
For enhanced security, we recommend running your Java application as a non-root
user. Create a dedicated user account and execute your application with reduced
privileges using this Dockerfile example:
...
Standard encoding is set to en_US.UTF-8, and other locale variables default to
en_US. JAVA_VERSION can help you identify the Java version, and JAVA_HOME is
set to /opt/java/openjdk.

```bash
JAVA_VERSION=jdk8u482-b08 # or a newer version depending on the build
...
the Temurin JDK 8 image. **This image runs as the root user! You should
...
reports, or wish to submit pull requests with improvements, you can engage with
us through the official Sourcemation GitHub repository.
...
solutions with comprehensive security risk assessment. We maintain
high-quality, production-ready container images. Detailed vulnerability
analysis and compliance reports for our images are accessible through the
