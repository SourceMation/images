#!/usr/bin/env bash
# ---------------------------------------------------
# Automated build process for the GCC 15
...
VERSION=$(git ls-remote --refs --tags https://github.com/gcc-mirror/gcc.git  | grep -o 'gcc-15[.0-9]*$' | sort --version-sort --reverse | tr -d 'gc-' | head -n 1)
