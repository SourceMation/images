#!/usr/bin/env bash
# -------------------------------------------------
# Automated build process for the ruby Docker image
...
VERSION=$(docker run --rm ${IMG} /bin/bash -c \
	"dnf module -y enable ruby:3.3 &> /dev/null; dnf check-update --refresh > /dev/null; dnf info --available ${APP}.${ARCH} | grep Version | awk '{print \$3}'")
