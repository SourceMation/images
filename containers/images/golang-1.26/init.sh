#!/usr/bin/env bash
# ---------------------------------------------------
# Automated build process for the golang 1.26 image
...
VERSION=$(git ls-remote --refs --tags https://github.com/golang/go.git | grep  -o 'go1.26[.0-9]*$' | sort --version-sort --reverse | tr -d 'go' | head -n 1)
