# Golang 1.26 packaged by Sourcemation
...
Sourcemation automation team. The version is 1.26.X and receives regular updates
...
Go 1.26 introduces several significant improvements and new features that enhance developer productivity and application performance:

- **"Green Tea" Garbage Collector**: Enabled by default, the new garbage collector reduces overhead by 10–40% for small objects and leverages vector instructions on modern CPUs for better performance.
- **New Packages**: Introduces the `crypto/hpke` package for hybrid encryption, along with experimental `simd/archsimd` for architecture-specific operations and `runtime/secret` for secure memory erasure.
- **Performance Improvements**: Significant performance boosts, including a 30% reduction in cgo call overhead, more efficient stack allocation for slices, and a faster `io.ReadAll` implementation.
- **Toolchain Updates**: A revamped `go fix` command for automated code modernization and a new default flame graph view in the `pprof` web UI.

These updates make Go 1.26 an excellent choice for modern cloud-native applications, microservices, and high-performance
systems.
...
```bash
docker run --rm -it sourcemation/golang-1.26:latest go version
```

To run the shell in the container, use the following command:

```bash
docker run --rm -it sourcemation/golang-1.26:latest /bin/bash
```
...
```bash
docker run --rm -p 8080:8080 -v "${PWD}:/app" -w "/app" -it sourcemation/golang-1.26:latest sh -c 'go run main.go'
```

Build a Go binary from the current directory and run it, this time with podman:

```bash
podman run --rm -p 8080:8080 -v "${PWD}:/app" -w "/app" -it sourcemation/golang-1.26:latest sh -c 'go build -o myapp && ./myapp'
```
...
GOPATH=/go
GOLANG_VERSION=1.26.X # example: 1.26.1
GOTOOLCHAIN=local
```
...
**Disclaimer:** The `sourcemation/golang-1.26` image is not affiliated with the
Go project or Google. The respective companies and organisations own the
trademarks mentioned in the offering. The `sourcemation/golang-1.26` image is a
