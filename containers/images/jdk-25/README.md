...
This image contains the Java Development Kit (JDK) 25 from the Eclipse Temurin
...
installation of JDK 25. It is built upon Sourcemation’s optimized Debian 13 Slim
...
drop-in replacement for the Temurin JDK 25 image.
...
```bash
docker run -it --rm sourcemation/jdk-25
```


This will start a `jshell` inside a container with JDK 25 installed and ready to
use.
...
```bash
docker run -it --rm -v /path/to/your/application:/app sourcemation/jdk-25 java -jar /app/your-application.jar
```
...
```bash
docker run -v /home/dev/mycerts:/certificates/ -e USE\_SYSTEM\_CA\_CERTS=1 sourcemation/jdk-25
```
...
the Temurin JDK 25 image. **This image runs as the root user\! You should
