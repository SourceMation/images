#!/usr/bin/env bash
# ---------------------------------------------------
# Automated build process for the NodeJS 26 image
...
NODE_VERSION=$(git ls-remote --refs --tags https://github.com/nodejs/node.git | grep  -o 'v26.[.0-9]*$'  | tr -d 'v' | sort --version-sort --reverse | head -1)
...
curl -LO https://github.com/nodejs/docker-node/raw/refs/heads/main/26/trixie-slim/docker-entrypoint.sh
