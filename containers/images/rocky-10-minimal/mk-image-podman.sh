#!/usr/bin/env bash
# This image scripts was used to generate EL10 images with podman
...
# Configure storage driver to vfs to support running inside Docker
...
dnf --disableplugin '*' --installroot "\$scratchmnt" --releasever=10 list --installed > /tmp/packages_list_yum
...
dnf --disableplugin='*' -y --installroot "\$scratchmnt" --releasever=10 clean all
set -x
...
rm -f base-image.tar base-image.tar.gz
