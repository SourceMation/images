...
def test_node_22_installed():
    ...
    assert result.stdout.startswith('v22.'), f"Expected Node.js version to start with v22., but got: {result.stdout}"
...
def test_run_simple_node_script_22():
    script_content = """
    console.log("Node.js version 22 is working!");
    """

    with open('test_script_22.js', 'w') as f:
        f.write(script_content)

    result = subprocess.run(['node', 'test_script_22.js'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    assert result.returncode == 0, f"Failed to run Node.js script: {result.stderr}"
    assert "Node.js version 22 is working!" in result.stdout, "Node.js script did not produce expected output"

    os.remove('test_script_22.js')


def test_install_npm_package_22():
