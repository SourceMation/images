...
def test_node_26_installed():
    ...
    assert result.stdout.startswith('v26.'), f"Expected Node.js version to start with v26., but got: {result.stdout}"
...
def test_run_simple_node_script_26():
    script_content = """
    console.log("Node.js version 26 is working!");
    """

    with open('test_script_26.js', 'w') as f:
        f.write(script_content)

    result = subprocess.run(['node', 'test_script_26.js'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    assert result.returncode == 0, f"Failed to run Node.js script: {result.stderr}"
    assert "Node.js version 26 is working!" in result.stdout, "Node.js script did not produce expected output"

    os.remove('test_script_26.js')


def test_install_npm_package_26():
