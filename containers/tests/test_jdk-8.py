...
def test_java_version():
    ...
    # For Java 8, the version string is usually "1.8.0_..."
    version_info = version_line.split(' ')[2].strip('"')
    assert version_info.startswith('1.8.'), f"Expected Java version to start with '1.8.', but got '{version_info}'"
...
def test_javac_installed():
    ...
    # javac -version prints to stderr in some versions, but to stdout in others.
    assert "javac" in result.stdout or "javac" in result.stderr, "Unexpected output from 'javac -version'"
