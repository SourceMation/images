...
def test_node_20_installed():
    ...
    assert result.stdout.startswith('v20.'), f"Expected Node.js version to start with v20., but got: {result.stdout}"
...
def test_run_simple_node_script_20():
    script_content = """
    console.log("Node.js version 20 is working!");
    """

    with open('test_script_20.js', 'w') as f:
        f.write(script_content)

    result = subprocess.run(['node', 'test_script_20.js'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    assert result.returncode == 0, f"Failed to run Node.js script: {result.stderr}"
    assert "Node.js version 20 is working!" in result.stdout, "Node.js script did not produce expected output"

    os.remove('test_script_20.js')


def test_install_npm_package_20():
