...
def test_os_release_is_rocky_10():
    ...
    assert release_info.get('VERSION_ID').startswith('10')
    assert "Rocky Linux 10" in release_info.get('PRETTY_NAME')
...
def test_redhat_release_file_is_correct():
    ...
    with open(release_file, 'r') as f:
        content = f.read()
        assert content.strip().startswith("Rocky Linux release 10")
...
def test_microdnf_can_make_cache():
    try:
        result = subprocess.run(['microdnf', 'makecache'], capture_output=True, text=True, timeout=180)
        assert result.returncode == 0, f"microdnf makecache failed with exit code {result.returncode}:\n{result.stderr}"
    except FileNotFoundError:
        pytest.fail("Command 'microdnf' not found in the container.")
    except subprocess.TimeoutExpired:
        pytest.fail("'microdnf makecache' timed out after 3 minutes.")
...
def test_merged_usr_filesystem_layout_extended(path):
    assert os.path.islink(path), f"Path '{path}' is not a symlink as expected in Rocky Linux 10."
