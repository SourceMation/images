...
def test_ownership_main_pgpool_directory():
    result = subprocess.run(["stat", "-c", "%U", "/opt/pgpool-II"], capture_output=True, text=True)
    assert result.returncode == 0, "stat command failed."
    assert result.stdout.strip() == "postgres", "ownership of the main pgpool directory is invalid"

def test_ownership_run_pgpool_directory():
    result = subprocess.run(["stat", "-c", "%U", "/var/run/pgpool"], capture_output=True, text=True)
    assert result.returncode == 0, "stat command failed."
    assert result.stdout.strip() == "postgres", "ownership of the run pgpool directory is invalid"
